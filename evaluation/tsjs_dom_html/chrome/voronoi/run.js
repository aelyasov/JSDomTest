// To complex to be analized, plus has a lot of dependencies. 

function run(freeze_frame_switch, canvas_width, canvas_heigth, mouse_x, previous_mouse_x, mouse_y, previous_mouse_y, num_vertices, is_mouse_down, blow_distance, follow_distance, stir_distance, viscosity) {

  var canvas = document.getElementById("main_canvas");
  var context = canvas.getContext("2d");

    if (freeze_frame_switch) {
        return;
    }
    ...
    var mouse_vx = mouse_x - previous_mouse_x;
    var mouse_vy = mouse_y - previous_mouse_y;
    ...
    edges.each(function(an_edge) {
        ...
        if (delaunay_switch) {
            ...
            if (color_switch && !an_edge.is_infinite_edge()) {
                if (left.payload["painted"] == false && !an_edge.onext().is_infinite_edge()) {
                    onext_velocity = 0;
                    [an_edge.org().payload["velocity"], an_edge.dest().payload["velocity"], an_edge.onext().dest().payload["velocity"]].each(function(velocity) {
                        onext_velocity += velocity / 3;
                    });
                ...
                if (right.payload["painted"] == false && !an_edge.oprev().is_infinite_edge()) {
                    oprev_velocity = 0;
                    [an_edge.org().payload["velocity"], an_edge.dest().payload["velocity"], an_edge.oprev().dest().payload["velocity"]].each(function(velocity) {
                        oprev_velocity += velocity / 3;
                    });
        ...
        if (voronoi_switch) {
            ...
            if (color_switch) {
                ...
                if (!org.is_infinity && right != null && left != null) {
                    draw_triangle(org, right, left, org_color);
                }
                if (!dest.is_infinity && right != null && left != null) {
                    draw_triangle(dest, left, right, dest_color);
                }
            }
        }
    });
