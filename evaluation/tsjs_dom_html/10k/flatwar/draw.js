function draw(gO, pa, enE, es, enR, enX, enY, pcX, pcY, pcR, enV, enS, l, bE, bX, bY, bDX, bDY, bR, bV, bs, boX, boY, mX, mY, mC, kO, pcS, pcG, pcGT, pcV, puX, puY, puR, puT, cs, cX, cY, cE, cR, cEN, cMR, ps, boR, ks, lK, lKA, rPC, gPC, bPC, tP) {
    ...
    function dB(bX, bY, tX, tY, rad, sid, lW) {
        ...
        var x1 = bX + rad * Math.cos(ang);
        var y1 = bY + rad * Math.sin(ang);
        c.beginPath();
        c.moveTo(x1, y1);
        for (var o = ang; o < d2r(360) + ang; o += dR) c.lineTo(bX + rad * Math.cos(o), bY + rad * Math.sin(o));
        c.lineTo(x1, y1);
        c.fillStyle = "#9a9a9a";
        c.fill();
        c.lineWidth = lW;
    ...
    if (!gO && !pa) {
        ...
        if (pcS < 1) gO = true;
        var ve = (kO[16]) ? pcV + 4 : pcV;
        var moX = 0;
        var moY = 0;
        if (kO[39] || kO[68]) moX = ve;
        if (kO[37] || kO[65]) moX = -ve;
        if (kO[38] || kO[87]) moY = ve;
        if (kO[40] || kO[83]) moY = -ve;
            ...
            if (bE[i]) {
                for (var j = 0; j < es; j++) {
                    if (enE[j]) {
                        ...
                        if (dist(bX[i], bY[i], mX, mY) > 600) bE[i] = false;
                    }
                }
            }
        }
        ...
        for (i = 0; i < cs; i++) {
            if (cE[i]) {
                var dis = dist(cX[i], cY[i], pcX, pcY);
                if (dis < cR[i] + pcR / 2) {
                    cE[i] = false;
                    ps++;
                    pcS += 0.001;
        ...
        for (i = 0; i < cs; i++) {
            if (cE[i]) {
                ...
                if (dist(cX[i], cY[i], pcX, pcY) < cMR) {
                    var disX = Math.abs(pcX - cX[i]);
                    var disY = Math.abs(pcY - cY[i]);
                    var dis = Math.sqrt(disX * disX + disY * disY);
                    var movX = disX * 10 / dis;
                    var movY = disY * 10 / dis;
        ...
        for (i = 0; i < es; i++) {
            if (enE[i]) {
                var dis = dist(enX[i], enY[i], pcX, pcY);
                if (dis < enR[i] + pcR) {
                    pcS -= 0.03;
                    enS[i] += 0.03;
                }
        ...
        for (i = 0; i < es; i++) {
            if (enE[i]) {
                if (enS[i] < 30) dB(enX[i], enY[i], pcX, pcY, enR[i], enS[i], 2);
                else cir(enX[i], enY[i], enR[i], "#9a9a9a", 2);
                var disX = Math.abs(pcX - enX[i]);
                var disY = Math.abs(pcY - enY[i]);
                var dis = Math.sqrt(disX * disX + disY * disY);
                var movX = disX * enV[i] / dis;
                var movY = disY * enV[i] / dis;
        ...
        if (puT == 0) var color = "#ff0000";
        else if (puT == 1) color = "#00ff00";
        else if (puT == 2) color = "#0000ff";
        cir(puX, puY, puR, color, 3);
        if (pcS < 30) dB(pcX, pcY, mX, mY, pcR, pcS, 2);
        else cir(pcX, pcY, pcR, "#9a9a9a", 2);
        if (!kO[16] && !kO[32]) {
            if (pcG > 18) pcG = 18;
            var w = pcR;
            var h = w / 8;
            ...
            for (i = 0; i < pcG / 2; i++) {
                ...
                var xE = (pcR + aR) * Math.cos(ang + d2r(70 - i * 40));
                var yE = (pcR + aR) * Math.sin(ang + d2r(70 - i * 40));
                ang = Math.atan2(mY - (pcY + yE), mX - (pcX + xE));
                if (mC) aB(pcX + xE + (w - h) * Math.cos(ang), pcY + yE + (w - h) * Math.sin(ang), ang, h - 3);
        ...
        for (var i = 0; i < kO.length; i++) kO[i] = undefined;
        info.style.left = 150;
        info.style.top = 150;
